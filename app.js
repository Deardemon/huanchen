...
/* ---------- Supabase 資料庫操作 ---------- */
async function dbLoadAgents(fallback) {
  try {
    const { data, error } = await supabase.from('agents').select('*').order('joined_at', { ascending: false });
    if (error || !data || data.length === 0) return fallback;
    return data.map(a => ({
      id: a.id, name: a.name, contact: a.contact,
      regions: a.regions || [], tags: a.tags || [],
      bio: a.bio || '', licenseNo: a.license_no || '', certNo: a.cert_no || '',
      firmName: a.firm_name || '', firmAddress: a.firm_address || '',
      guildName: a.guild_name || '', certPhoto: a.cert_photo || null,
      certPhotoName: a.cert_photo_name || '',
      points: a.points || 0, unlimitedUntil: a.unlimited_until || null,
      verified: a.verified || false, banned: a.banned || false,
      joinedAt: a.joined_at || null,
    }));
  } catch { return fallback; }
}

async function dbSaveAgent(agent) {
  try {
    await supabase.from('agents').upsert({
      id: agent.id, name: agent.name, contact: agent.contact,
      regions: agent.regions, tags: agent.tags, bio: agent.bio || '',
      license_no: agent.licenseNo || '', cert_no: agent.certNo || '',
      firm_name: agent.firmName || '', firm_address: agent.firmAddress || '',
      guild_name: agent.guildName || '', cert_photo: agent.certPhoto || '',
      cert_photo_name: agent.certPhotoName || '',
      points: agent.points, unlimited_until: agent.unlimitedUntil || null,
      verified: agent.verified, banned: agent.banned,
      joined_at: agent.joinedAt || Date.now(),
    });
  } catch (e) { console.error('dbSaveAgent error', e); }
}

async function dbLoadCases(fallback) {
  try {
    const { data, error } = await supabase.from('cases').select('*').order('ts', { ascending: false });
    if (error || !data) return fallback;
    return data.map(c => ({
      id: c.id, customerName: c.customer_name, customerContact: c.customer_contact,
      region: c.region, caseType: c.case_type, problemText: c.problem_text,
      status: c.status, matchedAgentId: c.matched_agent_id,
      declinedAgentIds: c.declined_agent_ids || [],
      threads: c.threads || {}, ts: c.ts,
    }));
  } catch { return fallback; }
}
...
async function dbSaveCase(c) {
  try {
    await supabase.from('cases').upsert({
      id: c.id, customer_name: c.customerName, customer_contact: c.customerContact,
      region: c.region, case_type: c.caseType, problem_text: c.problemText,
      status: c.status, matched_agent_id: c.matchedAgentId || null,
      declined_agent_ids: c.declinedAgentIds || [],
      threads: c.threads || {}, ts: c.ts,
    });
  } catch (e) { console.error('dbSaveCase error', e); }
}

async function dbLoadTransactions(fallback) {
  try {
    const { data, error } = await supabase.from('transactions').select('*').order('ts', { ascending: false });
    if (error || !data) return fallback;
    return data.map(t => ({
      id: t.id, agentId: t.agent_id, label: t.label, points: t.points, price: t.price, ts: t.ts,
    }));
  } catch { return fallback; }
}
...
async function dbSaveTransaction(tx) {
  try {
    await supabase.from('transactions').upsert({
      id: tx.id, agent_id: tx.agentId, label: tx.label, points: String(tx.points), price: tx.price, ts: tx.ts,
    });
  } catch (e) { console.error('dbSaveTransaction error', e); }
}

async function dbLoadVisits(fallback) {
  try {
    const { data, error } = await supabase.from('visits').select('*');
    if (error || !data) return fallback;
    const result = {};
    data.forEach(v => { result[v.date_str] = { customer: v.customer, agent: v.agent }; });
    return result;
  } catch { return fallback; }
}

async function dbUpsertVisit(dateStr, customer, agent) {
  try {
    await supabase.from('visits').upsert({ date_str: dateStr, customer, agent });
  } catch (e) { console.error('dbUpsertVisit error', e); }
}
...
function App() {
  ...
  useEffect(() => {
    (async () => {
      const a = await dbLoadAgents(SEED_AGENTS);
      const c = await dbLoadCases([]);
      const tx = await dbLoadTransactions([]);
      const v = await dbLoadVisits({});
      setAgents(a);
      setCases(c);
      setTransactions(tx);
      setVisits(v);
  ...
  const persistAgents = async (next) => {
    setAgents(next);
    try {
      await Promise.all(next.map(a => dbSaveAgent(a)));
    } catch (e) {
      console.error("persistAgents failed", e);
      showToast("資料儲存失敗，請稍後再試", "warn");
    }
  };
  const persistCases = async (next) => {
    setCases(next);
    try {
      await Promise.all(next.map(c => dbSaveCase(c)));
    } catch (e) { console.error("persistCases failed", e); }
  };
  const persistTransactions = async (next) => {
    setTransactions(next);
    try {
      const latest = next[next.length - 1];
      if (latest) await dbSaveTransaction(latest);
    } catch (e) { console.error("persistTransactions failed", e); }
  };
  const persistVisits = async (next) => {
    setVisits(next);
    try {
      const entries = Object.entries(next);
      await Promise.all(entries.map(([date, v]) => dbUpsertVisit(date, v.customer || 0, v.agent || 0)));
    } catch (e) { console.error("persistVisits failed", e); }
  };
